...
This package has source code and a build script (`tsup`), so it can be rebuilt on the server. Update the `build` script in your root `package.json` to build it before Next.js:

```jsonc
{
  "scripts": {
    "dev": "next dev",
    "build": "pnpm --filter @x402-stellar/paywall build && next build",
    "start": "next start"
  }
}
```
...
**`Module not found: Can't resolve '@x402-stellar/paywall'`** or **`'@x402-stellar/paywall/stellar'`** -- The paywall package needs to be built before Next.js. See [Step 9a](#9a-x402-stellarpaywall----add-a-prebuild-step). Make sure your build script is `"pnpm --filter @x402-stellar/paywall build && next build"`, not just `"next build"`.
