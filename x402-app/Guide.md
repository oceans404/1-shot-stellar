...
## Step 2: Install npm packages + vendor the Stellar paywall

### 2a. Install published x402 packages from npm

```bash
pnpm add -w @x402/core@~2.6.0 @x402/next@~2.6.0 @x402/stellar@~2.6.0
```
...
>
> **What this gives you:** `@x402/core` provides the protocol engine (resource server, facilitator client, types). `@x402/next` provides `paymentProxy` (proxy that gates pages behind payment), `withX402` (per-route wrapper for API routes), and `NextAdapter`. `@x402/stellar` provides the Stellar payment mechanism (`ExactStellarScheme`).

### 2b. Vendor the Stellar paywall package

> **Temporary workaround:** Steps 2b through 2f are only needed because `@x402-stellar/paywall` is not yet published to npm. Once it is, replace all of this with `pnpm add @x402-stellar/paywall` and skip to [Step 4](#step-4-configure-environment-variables).

The Stellar paywall (`@x402-stellar/paywall`) is **not yet published to npm**. You need to clone the [stellar/x402-stellar](https://github.com/stellar/x402-stellar) repo and copy it.
...
# Copy the paywall package and patches
...
```bash
ls packages/paywall/package.json
# Should exist
```

> **Why only the paywall?** The other x402 packages (`@x402/core`, `@x402/next`, `@x402/stellar`) are all published to npm and installed in Step 2a. Only the Stellar paywall UI package needs to be vendored.

### 2c. Patch the vendored paywall's package.json

The vendored paywall references `@x402/core` and `@x402/stellar` as `workspace:*`, which assumes they are local workspace packages. Since both are now on npm, update the references in `packages/paywall/package.json`:
...
Change `@x402/stellar` (in `devDependencies`):
```json
...
> **Why `~2.6.0`?** The tilde range (`~`) allows patch updates (2.6.x) but not minor updates. This matches the versions installed from npm and keeps the vendored paywall compatible.
...
> **Why only 1 entry?** Only the Stellar paywall package needs to be in the workspace. Everything else comes from npm.
...
2. Add `"@x402-stellar/paywall"` to your **existing** `"dependencies"` object (alongside `next`, `react`, etc.)
...
> **The `...` entries represent existing content** -- do not delete or modify them. Your project name may differ from `my-x402-app` depending on what you chose in Step 1 -- that is fine. You are adding one new key to `"dependencies"` and one new top-level `"pnpm"` key. (`@x402/core`, `@x402/next`, and `@x402/stellar` were already added by `pnpm add` in Step 2a.)
...
> **Temporary workaround:** This manual build step is only needed because `@x402-stellar/paywall` is not yet published to npm and must be vendored locally. Once it's published, this step goes away entirely.
...
> **Fix the tsconfig first:** The vendored paywall's `tsconfig.json` extends `../../tsconfig.base.json` from the original `x402-stellar` monorepo, which doesn't exist in your project. Replace it with a self-contained config:
>
> ```bash
> cat > packages/paywall/tsconfig.json << 'EOF'
> {
>   "compilerOptions": {
>     "target": "ES2022",
>     "module": "ESNext",
...
## Step 9: Prepare the build script for deployment

> **Temporary workaround:** Step 9a exists because `@x402-stellar/paywall` is not yet published to npm. Once it is, this step is unnecessary — npm packages resolve normally in any CI/deployment environment.

The vendored paywall package (`@x402-stellar/paywall`) needs its `dist/` directory to exist before Next.js builds. Locally you built the paywall in Step 3, but on a CI/deployment platform like Vercel, those dist files won't exist unless you handle them explicitly:

### 9a. `@x402-stellar/paywall` -- add a prebuild step

This package has source code and a two-step build (codegen + tsup), so it can be rebuilt on the server. Update the `build` script in your root `package.json` to run both steps before Next.js:

```jsonc
{
  "scripts": {
    "dev": "next dev",
    "build": "pnpm --filter @x402-stellar/paywall codegen && pnpm --filter @x402-stellar/paywall build && next build",
    "start": "next start"
  }
}
```

The `codegen` step generates `src/gen/template.ts` (the paywall HTML template), and `build` compiles everything with tsup. Both are required -- without codegen, tsup will fail with `Could not resolve "./gen/template.js"`.

Without this, the deployment will fail with:
```
Module not found: Can't resolve '@x402-stellar/paywall'
Module not found: Can't resolve '@x402-stellar/paywall/stellar'
```

or:
```
Could not resolve "./gen/template.js"
```

### 9b. Verify the production build

Now verify the production build succeeds:
...
After verifying the app works locally, you can deploy to Vercel.

### Prerequisites

Make sure you have completed [Step 9](#step-9-prepare-the-build-script-for-deployment) -- the build script and `.gitignore` changes are required for the deployment to succeed.

### Deploy

1. Push your code to a GitHub repository.

2. Import the project in [Vercel](https://vercel.com/new). Set the **Root Directory** to `x402-app/video-paywall-app` (or wherever your Next.js app lives within the repo).

3. Add your environment variables in Vercel's project settings (the same values from your `.env.local`):
   - `FACILITATOR_URL`
   - `FACILITATOR_API_KEY`
   - `SERVER_STELLAR_ADDRESS`
   - `PAYMENT_PRICE`
   - `STELLAR_NETWORK` (optional, defaults to `stellar:testnet`)
   - `SOROBAN_RPC_URL` (required only for mainnet)

4. Deploy. The build command (`pnpm build`) will automatically build the paywall package first, then build the Next.js app.

### Troubleshooting deployment

**`Module not found: Can't resolve '@x402-stellar/paywall'`** or **`'@x402-stellar/paywall/stellar'`** -- The paywall package needs to be built before Next.js. See [Step 9a](#9a-x402-stellarpaywall----add-a-prebuild-step). Make sure your build script includes both `codegen` and `build` steps: `"pnpm --filter @x402-stellar/paywall codegen && pnpm --filter @x402-stellar/paywall build && next build"`.

**`Could not resolve "./gen/template.js"`** -- The codegen step is missing from the build script. The paywall's `tsup` build depends on `src/gen/template.ts`, which is generated by the `codegen` script. Make sure `codegen` runs before `build` in [Step 9a](#9a-x402-stellarpaywall----add-a-prebuild-step).
...
my-x402-app/
  ...
  pnpm-workspace.yaml           # Workspace config (1 entry)
...
**Symptom:** `pnpm install` fails with errors about `@x402/core` or `@x402/stellar` not being found in the workspace.

**Cause:** The vendored `@x402-stellar/paywall` still has `"@x402/core": "workspace:*"` or `"@x402/stellar": "workspace:*"` in its `package.json`.

**Fix:** Change both `"@x402/core": "workspace:*"` and `"@x402/stellar": "workspace:*"` to `"~2.6.0"` in `packages/paywall/package.json`. See [Step 2c](#2c-patch-the-vendored-paywalls-packagejson).
...
- `@x402-stellar/paywall` is **not on npm**. It must be vendored from [stellar/x402-stellar](https://github.com/stellar/x402-stellar).
- `@x402/core`, `@x402/next`, and `@x402/stellar` **are on npm** at version 2.6.0. Do not vendor them.
