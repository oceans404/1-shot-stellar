...
> **Temporary workaround:** Steps 2b through 2f are only needed because `@x402/stellar` and `@x402-stellar/paywall` are not yet published to npm. Once they are, replace all of this with `pnpm add @x402/stellar @x402-stellar/paywall` and skip to [Step 4](#step-4-configure-environment-variables).
...
> **Temporary workaround:** This manual build step is only needed because `@x402-stellar/paywall` is vendored locally. Once it's published to npm, this step goes away entirely.
...
> **Temporary workaround:** Steps 9a and 9b exist because the workspace packages aren't published to npm. Once `@x402/stellar` and `@x402-stellar/paywall` are on npm, both steps are unnecessary — npm packages resolve normally in any CI/deployment environment.

Both workspace packages (`@x402-stellar/paywall` and `@x402/stellar`) need their `dist/` directories to exist before Next.js builds. Locally you built the paywall in Step 3, but on a CI/deployment platform like Vercel, those dist files won't exist unless you handle them explicitly. Each package requires a different fix:
...
### 9b. `@x402/stellar` -- commit the prebuilt dist to git

This vendor package is different: it contains **only prebuilt `dist/` files** with no source code or build script. There is nothing to rebuild -- the `dist/` must be checked into git so it exists when the repo is cloned during deployment.

The root `.gitignore` has a blanket `dist/` rule that prevents this. Add an exception for the vendored package (in the repo root `.gitignore`, not the Next.js project's):

```gitignore
dist/
# Allow vendored pre-built packages (no source to rebuild from)
!x402-app/video-paywall-app/vendors/**/dist/
```

Then force-add the vendor dist (needed because git still remembers the previous ignore rule):

```bash
git add -f vendors/x402/typescript/packages/mechanisms/stellar/dist/
```

Without this, the deployment will fail with:
```
Module not found: Can't resolve '@x402/stellar/exact/server'
```

> **Why can't we just add a build step for `@x402/stellar` like we did for the paywall?** Because the vendored `@x402/stellar` directory only contains `dist/` and `package.json` -- there is no `src/` directory, no `tsconfig.json`, and no build script. It was vendored as a prebuilt artifact from the [stellar/x402-stellar](https://github.com/stellar/x402-stellar) repository. The only way to make it available in CI is to commit the `dist/` files.

### 9c. Verify the production build
...
1. Push your code to a GitHub repository (make sure the vendor `dist/` files from Step 9 are committed).
...
**`Module not found: Can't resolve '@x402/stellar/exact/server'`** -- The `@x402/stellar` vendor package has no source code -- its prebuilt `dist/` must be committed to git. See [Step 9b](#9b-x402stellar----commit-the-prebuilt-dist-to-git). Make sure you added the `.gitignore` exception and ran `git add -f` on the vendor dist.
